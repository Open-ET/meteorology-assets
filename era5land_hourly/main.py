...
def era5land_hourly_export(
        tgt_dt,
        region=None,
        fill_edge_cells=0,
        overwrite_flag=False,
):
    """Export hourly ERA5-Land image for a single date

    Parameters
    ----------
    tgt_dt : datetime
    region : {'global', 'eu', 'na', 'sa', 'hawaii'}, optional
    ...
    if not region or (region.lower() == 'global'):
        logging.info(f'Export ERA5-Land hourly image - {tgt_dt.strftime("%Y-%m-%d %H00")}')
        export_name = f'openet_meteo_era5land_hourly_{tgt_dt.strftime("%Y%m%d%H")}'
        # DEADBEEF - No longer saving ERA5-Land assets as COGs
        # bucket_img = f'{BUCKET_FOLDER}/hourly/{tgt_dt.strftime(ASSET_DT_FMT)}.tif'
        # bucket_json = bucket_img.replace('.tif', '_properties.json')
        asset_id = f'{ASSET_FOLDER}/hourly/{tgt_dt.strftime(ASSET_DT_FMT)}'
    else:
        logging.info(f'Export ERA5-Land hourly image - {region} - '
                     f'{tgt_dt.strftime("%Y-%m-%d %H00")}')
        logging.debug(f'  Region: {region}')
        export_name = f'openet_meteo_era5land_{region}_hourly_{tgt_dt.strftime("%Y%m%d%H")}'
        # DEADBEEF - No longer saving ERA5-Land assets as COGs
    ...
    if src_count == 0:
        ...
        logging.info('  Too many source images')
        return f'{export_name} - Too many source images, skipping\n'
    ...
    eto = openet.refetgee.Hourly.era5_land(src_img, fill_edge_cells=fill_edge_cells).eto
    etr = openet.refetgee.Hourly.era5_land(src_img, fill_edge_cells=fill_edge_cells).etr

    # # Compute the hourly meteorology variables
    # temp = src_img.select(['temperature_2m'])
    # tdew = src_img.select(['dewpoint_temperature_2m'])
    # wind = (
    #     src_img.select(['u_component_of_wind_10m']).pow(2)
    #     .add(src_img.select(['v_component_of_wind_10m']).pow(2))
    #     .sqrt()
    #     .rename(['wind_10m'])
    # )
    # srad = src_img.select(['surface_solar_radiation_downwards_hourly'])
    # prcp = src_img.select(['total_precipitation_hourly'])
    ...
    output_img = (
        ee.Image([
            # tmax, tmin, tmean, tdew, wind, srad, prcp,
            eto, etr
        ])
        .rename([
            # 'temperature_2m_max',
...
def era5land_hourly_asset_dates(
        start_dt,
        end_dt,
        region=None,
        overwrite_flag=False,
):
    """Identify dates of missing ERA5-Land hourly assets
    ...
    """
    logging.info('\nBuilding ERA5-Land hourly asset date list')

    # TODO: Should probably pass in the asset_coll_id instead of building from region
    if not region or (region.lower() == 'global'):
        asset_coll_id = f'{ASSET_FOLDER}/hourly'
    else:
        asset_coll_id = f'{ASSET_FOLDER}/{region}/hourly'

    task_id_re = re.compile(f'openet_meteo_era5land(_\w+)?_hourly_(?P<date>\d{{10}})$')
...
def cron_scheduler(request):
    ...
    response = 'Queue ERA5-Land hourly asset export tasks\n'
    args = {}
    ...
    # Process each date
    for tgt_dt in sorted(era5land_hourly_asset_dates(**args), reverse=reverse_flag):
        # logging.info(f'Date: {ingest_dt.strftime("%Y-%m-%d")}')
        # response += 'Date: {}\n'.format(ingest_dt.strftime('%Y-%m-%d'))
        response += era5land_hourly_export(
            tgt_dt,
            region=args['region'],
            fill_edge_cells=fill_edge_cells,
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Build ERA5-Land hourly assets',
...
if __name__ == '__main__':
    ...
    #
    # ingest_dt_list = era5land_hourly_asset_dates(
