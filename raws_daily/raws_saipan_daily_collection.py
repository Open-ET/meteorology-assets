...
ASSET_DT_FMT = '%Y%m%d'
PROJECT_NAME = 'openet'
SOURCE_COLL_ID = 'projects/openet/assets/meteorology/raws/saipan/hourly'
#ANCILLARY_FOLDER = 'projects/openet/assets/meteorology/raws/saipan/ancillary'
START_DAY_OFFSET = 90
END_DAY_OFFSET = 0
START_HOUR_OFFSET = -12
NODATA = -9999
...
def raws_saipan_daily_export(tgt_dt, refet_timestep='hourly', overwrite_flag=False):
    """Export daily RAWS Saipan image for a single date
    ...
    logging.info(f'Export RAWS Saipan daily image - {tgt_dt.strftime("%Y-%m-%d")}')
    export_name = f'openet_meteo_raws_saipan_daily_{tgt_dt.strftime("%Y%m%d")}'
    ...
    if refet_timestep.lower() in ['hour', 'hourly']:
        # Sum precomputed hourly reference ET to daily
        ...
        #     r = openet.refetgee.Hourly(
        #         tmean=img.select(['TEMPERATURE']),
        #         ea=img.select(['EA']).divide(1000),
        #         uz=img.select(['WIND']),
        #         zw=wind_height,
        #         rs=img.select(['RS']).multiply(0.0036),
        #         elev=elevation,
        #         lat=latitude,
        #         lon=longitude,
        #         #elev=ee.Image('projects/openet/assets/meteorology/raws/saipan/ancillary/elevation'),
        #         #lat=ee.Image('projects/openet/assets/meteorology/raws/saipan/ancillary/latitude'),
        #         #lon=ee.Image('projects/openet/assets/meteorology/raws/saipan/ancillary/latitude'),
        #     )
        ...
        # Note, this calculation is using the solar radiation computed from total cloud cover
        refet_obj = openet.refetgee.Daily(
            tmax=src_coll.select(['TEMPERATURE']).max(),
            tmin=src_coll.select(['TEMPERATURE']).min(),
            ea=src_coll.select(['EA']).mean().divide(1000),
            rs=src_coll.select(['RS']).sum().multiply(0.0864),
            uz=src_coll.select(['WIND']).mean(),
            zw=wind_height,
            elev=elevation,
            lat=latitude,
            # elev=ee.Image('projects/openet/assets/meteorology/raws/saipan/ancillary/elevation'),
    ...
    output_img = (
        ee.Image([
            src_coll.select(['TEMPERATURE']).mean(),
            src_coll.select(['TEMPERATURE']).max(),
            src_coll.select(['TEMPERATURE']).min(),
            src_coll.select(['RH']).mean(),
            src_coll.select(['SPFH']).mean(),
            src_coll.select(['EA']).mean(),
            src_coll.select(['PRESSURE']).mean(),
            src_coll.select(['WIND']).mean(),
            src_coll.select(['RS']).sum(),
            src_coll.select(['PRECIP']).sum(),
            eto,
            etr,
        ])
        .rename([
...
def daily_asset_dates(start_dt, end_dt, overwrite_flag=False):
    ...
    task_id_re = re.compile(f'openet_meteo_raws_saipan_(_\w+)?_daily_(?P<date>\d{{8}})$')
...
def cron_scheduler(request):
    ...
    response = 'Queue RAWS Saipan daily asset export tasks\n'
    args = {}
    ...
    for ingest_dt in sorted(daily_asset_dates(**args), reverse=reverse_flag):
        ...
        response += raws_saipan_daily_export(
            ingest_dt,
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Build RAWS Saipan daily assets',
